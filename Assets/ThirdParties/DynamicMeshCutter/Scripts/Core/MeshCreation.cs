    ...
    public static class MeshCreation
    {
        ...
        public static MeshCreationData CreateObjects(Info info, Material defaultMaterial, int vertexCreationThreshold)
        {
            ...
            for (int i = 0; i < createdMeshes.Length; i++)
            {
                ...
                if(i==0) {
                    parent.GetComponent<Rigidbody>().AddExplosionForce(30,info.Plane.WorldNormal * 4 , 100,3,ForceMode.Impulse);
                    // parent.GetComponent<Rigidbody>().AddForce(info.Plane.WorldNormal * 4 , ForceMode.Impulse);
                } else {
                    parent.GetComponent<Rigidbody>().AddExplosionForce(30,info.Plane.WorldNormal * -4 , 100,3,ForceMode.Impulse);
                    // parent.GetComponent<Rigidbody>().AddForce(info.Plane.WorldNormal * -4 , ForceMode.Impulse);
                }
